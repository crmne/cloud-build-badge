...
run_test() {
  for status in cancelled failure internal_error queued status_unknown success timeout working; do
    echo testing build status \"$status\"
    gcloud functions call cloud-build-badge --data "$(event $status)"
    curl -sS https://storage.googleapis.com/${BADGES_BUCKET}/builds/test-repo/branches/master.svg |
      xargs echo xq -e \
        --arg status "${status//_/ }" \
